...
static const value_string hcrt_dos[] = {
    {0x0, "not DO"},
    {0x1, "DO"},
    {0, NULL}
};
    ...
    int type, int addr_mode, int adl, int body_len)
{
    ...
    proto_tree *hcrt_body_tree = NULL;
    gint i;
...
static int dissect_hcrt_header(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree,
    int* offset, guint8 b0_first, guint8 b0_current)
{
    proto_item *ti_hdr = NULL;
    proto_tree *hcrt_hdr_tree = NULL;
    guint8 last;
    guint8 type;
    ...
    if (b0_first != b0_current) {
        expert_add_info_format(pinfo, hcrt_hdr_tree, PI_MALFORMED, PI_ERROR,
            "Invalid Byte 0 in Header. Must be equal in all HCrt messages. "
                "Expected: %.2X, got: %.2X", b0_first, b0_current);
    }

    type = (b0_current & 0x78) >> 4;

    /* == Byte 0 == */
    /* TAG */
    proto_tree_add_bits_item(hcrt_hdr_tree, hf_hcrt_message_tag, tvb,
        (*offset * 8) + 4, 4, ENC_BIG_ENDIAN);
    /* Message Type */
    proto_tree_add_bits_item(hcrt_hdr_tree, hf_hcrt_message_type, tvb,
        (*offset * 8) + 2, 2, ENC_BIG_ENDIAN);
    /* Addressing Mode */
    proto_tree_add_bits_item(hcrt_hdr_tree, hf_hcrt_am, tvb,
        (*offset * 8) + 1, 1, ENC_BIG_ENDIAN);
    /* Discovery Operation */
    proto_tree_add_bits_item(hcrt_hdr_tree, hf_hcrt_do, tvb,
    ...
    if (type != HCRT_RESPONSE) {
        /* 1st DWORD enable */
        proto_tree_add_bits_item(hcrt_hdr_tree, hf_hcrt_1st_dword_enable, tvb,
            (*offset * 8) + 4, 4, ENC_BIG_ENDIAN);
    } else {
        /* Response Code */
        proto_tree_add_bits_item(hcrt_hdr_tree, hf_hcrt_resp_code, tvb,
            (*offset * 8) + 4, 4, ENC_BIG_ENDIAN);
    }

    if (type != HCRT_RESPONSE) {
        /* Last DWORD enable */
        proto_tree_add_bits_item(hcrt_hdr_tree, hf_hcrt_last_dword_enable, tvb,
            (*offset * 8) + 0, 4, ENC_BIG_ENDIAN);
    }
    (*offset)++;
    ...
    /* Last */
    proto_tree_add_bits_item(hcrt_hdr_tree, hf_hcrt_last, tvb, (*offset * 8) + 8,
        1, ENC_BIG_ENDIAN);

    (*offset)++;

    /* last */
    last = (tvb_get_guint8(tvb, *offset) & 0x80) >> 7;
    (*offset)++;
    return last;
}

/* Return true if this is the last message */
static int dissect_hcrt_message(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree,
    int* offset, guint8 b0_first, int i)
{
    ...
    guint addr_mode;
    guint body_len;
    proto_item *ti_msg = NULL;
    proto_tree *hcrt_msg_tree = NULL;
    guint8 b0_current;
    int type;
    ...
    ti_msg = proto_tree_add_string_format(tree, hf_hcrt_message, tvb, *offset,
        HCRT_HDR_LEN + body_len, "", "Message %d", i);
    hcrt_msg_tree = proto_item_add_subtree(ti_msg, ett_hcrt_msg);
